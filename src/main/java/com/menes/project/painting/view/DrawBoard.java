...
public class DrawBoard extends JPanel {
    ...
    public DrawBoard(StatusBar c) {
        ...
        icon = new ImageIcon("./icons/undo2.png");
        ...
        icon = new ImageIcon("./icons/redo2.png");
        ...
        MouseAdapter click = new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                switch (toolType) {
                    case "Select" -> checkSelect(e.getPoint());
                    case "Move" -> {
                        for (AShape a : shapes) {
                            if (a.getShowPoints())
                    ...
                    case "Paste" -> {
                        ...
                        if (copy) {
                            for (AShape aShape : shapes) {
                                if (aShape.getShowPoints()) {

                                    switch (aShape.getTypeShape()) {
                                        case "Circle" -> fakeShape = new Circle(aShape.getRoot(), aShape.getColor(),
                                                aShape.getDimension());
                                        case "Oval" -> fakeShape = new Oval(aShape.getRoot(), aShape.getColor(),
                                                aShape.getDimension());
                                        case "Rectangle" ->
                                                fakeShape = new Rectangle(aShape.getRoot(), aShape.getColor(),
                                                        aShape.getDimension());
                                        case "Square" -> fakeShape = new Square(aShape.getRoot(), aShape.getColor(),
                                                aShape.getDimension());
                                        case "Line" -> fakeShape = new Line(aShape.getRoot(), aShape.getColor(),
                                                aShape.getDimension());
                                        case "Triangle" -> fakeShape = new Triangle(aShape.getRoot(), aShape.getColor(),
                                                aShape.getDimension());
                                    }
...
//				shape = new Pencil(e.getPoint(), getColor(), size);
                if (started) {
                    switch (type) {
                        case "Line" -> shape = new Line(e.getPoint(), getColor(), size);
                        case "Circle" -> shape = new Circle(e.getPoint(), getColor(), size);
                        case "Oval" -> shape = new Oval(e.getPoint(), getColor(), size);
                        case "Rectangle" -> shape = new Rectangle(e.getPoint(), getColor(), size);
                        case "Pencil" -> shape = new Pencil(e.getPoint(), getColor(), size);
                        case "Eraser" -> shape = new Eraser(e.getPoint(), getBackground(), size);
                        case "Square" -> shape = new Square(e.getPoint(), getColor(), size);
                        case "Triangle" -> shape = new Triangle(e.getPoint(), getColor(), size);
                        case "Fill" -> {
                            if (shapes.size() > 0) {
                                for (AShape a : shapes) {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (highlight) {
            if (twoLines.size() > 0) {
                ...
                g.drawLine(twoLines.get(twoLines.size() - 1).x, twoLines.get(twoLines.size() - 1).y - 1500,
                        twoLines.get(twoLines.size() - 1).x, twoLines.get(twoLines.size() - 1).y + 1500);
                g.drawLine(twoLines.get(twoLines.size() - 1).x - 1500, twoLines.get(twoLines.size() - 1).y,
                        twoLines.get(twoLines.size() - 1).x + 1500, twoLines.get(twoLines.size() - 1).y);
            }
        }
